...
import { Search, Menu, X, Shield, LogOut, User, ChevronDown, Megaphone } from "lucide-react";
import { useGetSettings, useListAnnouncements } from "@workspace/api-client-react";
...
export function Navbar() {
  ...
  const showBanner = !bannerDismissed && (
    (settings?.announcementBannerEnabled && settings.announcementBannerText) ||
    activeAnnouncements.length > 0
  );
  const bannerText = settings?.announcementBannerEnabled && settings.announcementBannerText
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showBanner && bannerText && (
          <motion.div
            ...
            className="fixed top-0 left-0 right-0 z-[60] bg-cyan-500 text-black overflow-hidden"
          >
            <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-4">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <Megaphone size={14} className="flex-shrink-0" />
                <span className="text-xs sm:text-sm font-bold truncate">{bannerText}</span>
              ...
              <button onClick={() => setBannerDismissed(true)} className="flex-shrink-0 hover:bg-black/10 rounded p-0.5 transition-colors">
                <X size={14} />
              </button>
            </div>
          </motion.div>
      ...
      <nav className={`fixed left-0 right-0 z-50 border-b border-cyan-500/20 bg-black/80 backdrop-blur-xl transition-all duration-200 ${showBanner && bannerText ? "top-8" : "top-0"}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-1">
              ...
              <a
                href={settings?.discordInvite || "https://discord.gg/versustiers"}
                target="_blank"
